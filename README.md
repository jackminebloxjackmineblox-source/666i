--[[
    ...
    ✅ ESP sem duplicatas / sem erro após morte
...
local AlcanceAimbot = 200 -- Alcance máximo em metros
local Hue = 0
local FPS = 0
...
local SombraJanela = CriarElemento("ImageLabel", {
...
local function AlternarGUI()
    ...
    if GuiVisivel then
        ...
        TweenService:Create(Janela, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
            Position = UDim2.new(0.02, 0, 0.1, -500),
            Transparency = 0.3
        }):Play()
        task.wait(0.4)
...
-- ✅ ARRASTE CORRIGIDO: Janela + Botão Abrir
...
-- Abas
local PainelAbas = CriarElemento("Frame", {
    Size = UDim2.new(1,-10,0,35),
    Position = UDim2.new(0,5,0,45),
    BackgroundTransparency = 1
}, Janela)

local BotaoAba1 = CriarElemento("TextButton", {
...
local BotaoAba2 = CriarElemento("TextButton", {
    Size = UDim2.new(0.23,0,1,0),
...
local BotaoAba3 = CriarElemento("TextButton", {
    Size = UDim2.new(0.23,0,1,0),
...
local BotaoAba4 = CriarElemento("TextButton", {
    Size = UDim2.new(0.23,0,1,0),
...
-- Áreas das abas
local ConteudoAba1 = CriarElemento("ScrollingFrame", {
    Size = UDim2.new(1,-10,1,-85),
    Position = UDim2.new(0,5,0,80),
    BackgroundTransparency = 1,
    ScrollBarThickness = 4,
    ScrollBarImageColor3 = Color3.fromRGB(80,80,150),
    CanvasSize = UDim2.new(0,0,0,320),
    Visible = true
}, Janela)

local ConteudoAba2 = CriarElemento("Frame", {
    Size = UDim2.new(1,-10,1,-85),
    Position = UDim2.new(0,5,0,80),
...
local ConteudoAba3 = CriarElemento("Frame", {
    Size = UDim2.new(1,-10,1,-85),
    Position = UDim2.new(0,5,0,80),
    BackgroundColor3 = Color3.fromRGB(25,25,45),
    BackgroundTransparency = 0.3,
...
local ConteudoAba4 = CriarElemento("Frame", {
    Size = UDim2.new(1,-10,1,-85),
    Position = UDim2.new(0,5,0,80),
    BackgroundColor3 = Color3.fromRGB(25,25,45),
    BackgroundTransparency = 0.3,
...
local function MostrarAba(n)
    ...
    ConteudoAba1.Visible = n == 1
    ConteudoAba2.Visible = n == 2
    ConteudoAba3.Visible = n == 3
    ConteudoAba4.Visible = n == 4
end
...
-- ⚙️ ABA GERAL
local LayoutAba1 = CriarElemento("UIListLayout", {
    Padding = UDim.new(0, 12),
    HorizontalAlignment = Enum.HorizontalAlignment.Center
}, ConteudoAba1)

-- Controle de velocidade
local GrupoVel = CriarElemento("Frame", {
...
CriarElemento("TextLabel", {
    ...
    Position = UDim2.new(0.025, 0, 0, 5),
    BackgroundTransparency = 1,
    Text = "🏃 VELOCIDADE",
    TextColor3 = Color3.fromRGB(220,220,255),
    TextSize = 13,
...
local BotaoVel = CriarElemento("TextButton", {
    Size = UDim2.new(0.2, 0, 0, 35),
    Position = UDim2.new(0.77, 0, 0, 35),
    BackgroundColor3 = Color3.fromRGB(60,120,90),
    Text = "APLICAR",
    TextColor3 = Color3.new(1,1,1),
    TextSize = 12,
    Font = Enum.Font.GothamBold
...
-- Lista de jogadores
local GrupoJog = CriarElemento("Frame", {
    Size = UDim2.new(0.95, 0, 0, 170),
    BackgroundColor3 = Color3.fromRGB(35,35,65),
    BackgroundTransparency = 0.2
}, ConteudoAba1)
...
CriarElemento("TextLabel", {
    ...
    Position = UDim2.new(0.025, 0, 0, 5),
    BackgroundTransparency = 1,
    Text = "👥 JOGADORES | TELEPORTE ATRÁS",
    TextColor3 = Color3.fromRGB(220,220,255),
    TextSize = 13,
    Font = Enum.Font.GothamBold
}, GrupoJog)

local ListaJog = CriarElemento("ScrollingFrame", {
    Size = UDim2.new(0.95, 0, 0, 130),
...
local LayoutLista = CriarElemento("UIListLayout", {
...
local function AtualizarLista()
    ...
    for _,j in ipairs(Players:GetPlayers()) do
        if j ~= LocalPlayer then
            local Linha = CriarElemento("Frame", {
            ...
            local Foto = CriarElemento("ImageLabel", {
                Size = UDim2.new(0, 26, 0, 26),
                Position = UDim2.new(0, 3, 0, 3),
                BackgroundColor3 = Color3.fromRGB(60,60,100),
                Image = Players:GetUserThumbnailAsync(j.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
            }, Linha)
            CriarElemento("UICorner", {CornerRadius = UDim.new(1, 0)}, Foto)

            local Nome = CriarElemento("TextLabel", {
            ...
            local BotaoTp = CriarElemento("TextButton", {
            ...
            Linha.Position = UDim2.new(0, -100, 0, 0)
            TweenService:Create(Linha, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Position = UDim2.new(0, 0, 0, 0)}):Play()
        end
    end
...
-- 👁️ ABA ESP
local BotaoESP = CriarElemento("TextButton", {
...
local function AlternarESP()
    ...
    TweenService:Create(BotaoESP, TweenInfo.new(0.2), {
        BackgroundColor3 = ESPAtivo and Color3.fromRGB(80,140,220) or Color3.fromRGB(60,90,140)
    }):Play()
...
-- 🎯 ABA AIMBOT
local BotaoAim = CriarElemento("TextButton", {
    Size = UDim2.new(0.8, 0, 0, 40),
    Position = UDim2.new(0.1, 0, 0.1, 0),
    BackgroundColor3 = Color3.fromRGB(140,60,90),
    Text = "ATIVAR AIMBOT",
    TextColor3 = Color3.new(1,1,1),
    TextSize = 13,
    Font = Enum.Font.GothamBold
...
local CaixaAlcance = CriarElemento("TextBox", {
    Size = UDim2.new(0.55, 0, 0, 35),
    Position = UDim2.new(0.1, 0, 0.4, 0),
    BackgroundColor3 = Color3.fromRGB(50,50,90),
    Text = "200",
    PlaceholderText = "Ex: 50 a 500",
    TextColor3 = Color3.new(1,1,1),
    TextSize = 13,
    Font = Enum.Font.Gotham,
    ClearTextOnFocus = false
...
local BotaoAlcance = CriarElemento("TextButton", {
    Size = UDim2.new(0.25, 0, 0, 35),
...
local function AlternarAimbot()
    ...
    TweenService:Create(BotaoAim, TweenInfo.new(0.2), {
        BackgroundColor3 = AimbotAtivo and Color3.fromRGB(220,80,120) or Color3.fromRGB(140,60,90)
    }):Play()
...
task.spawn(function()
    while task.wait(0.015) do -- Intervalo suave
        if not AimbotAtivo or not Character or not Humanoid or not RootPart or Humanoid.Health <= 0 then
            task.wait(0.1)
            continue
        end
        ...
        for _, jogador in ipairs(Players:GetPlayers()) do
            ...
            local posCabeca = cabeca.Position
            local dist = (RootPart.Position - posCabeca).Magnitude

            -- Verifica alcance e se está visível
            if dist <= AlcanceAimbot then
                local direcao = (posCabeca - posCamera).Unit
                local raio = Ray.new(posCamera, direcao * dist)
                local ignorar = {Character, Camera, workspace.Terrain}
                local parteAtingida = Workspace:FindPartOnRayWithIgnoreList(raio, ignorar)

                if parteAtingida and parteAtingida:IsDescendantOf(char) then
        ...
        -- Mira suave no alvo
        if melhorAlvo then
            Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(posCamera, melhorAlvo.Position), 0.25)
        end
    end
end)

-- 📌 ESP SEM DUPLICATAS E FUNCIONA APÓS MORTE
task.spawn(function()
    local desenhos = {}
    while task.wait(0.03) do
        ...
        if not Character or not RootPart or not Humanoid then task.wait(0.1); continue end
        ...
        for _, j in ipairs(Players:GetPlayers()) do
            ...
            local c = j.Character
            if not c or not c:FindFirstChild("HumanoidRootPart") or not c:FindFirstChild("Head") then continue end
            if c.Humanoid.Health <= 0 then continue end
            ...
            if not visCima or posCima.Z < 1 then continue end

            -- Texto do nome com marcação de dono
            local textoNome = j.Name
            local corNome = Color3.new(1, 1, 1)
            if j.Name == DONO_NOME then
                textoNome = j.Name .. " | 👑 DONO"
                corNome = Color3.fromRGB(255, 215, 0)
            end

            local txtNome = Drawing.new("Text")
            txtNome.Text = textoNome
            txtNome.Position = Vector2.new(posCima.X, posCima.Y)
            txtNome.Color = corNome
            txtNome.Size = 14
            txtNome.Center = true
            txtNome.Outline = true
            txtNome.OutlineColor = Color3.new(0,0,0)
            txtNome.Visible = true
            table.insert(desenhos, txtNome)

            -- Desenha ESP se ativado
            if ESPAtivo and visBase and posBase.Z > 1 then
                local dist = math.floor((rootAlvo.Position - RootPart.Position).Magnitude)
                ...
                mira.From = centroTela
                mira.To = Vector2.new((esq+dir)/2, bai)
                mira.Color = CorRGB
                mira.Thickness = 1
                mira.Visible = true
                table.insert(desenhos, mira)

                -- Distância
                local txtDist = Drawing.new("Text")
                txtDist.Text = dist .. "m"
                txtDist.Position = Vector2.new((esq+dir)/2, bai + 12)
                txtDist.Color = CorRGB
                txtDist.Size = 12
                txtDist.Center = true
                txtDist.Outline = true
                txtDist.OutlineColor = Color3.new(0,0,0)
                txtDist.Visible = true
                table.insert(desenhos, txtDist)
            end
        end
    end
end)
...
RunService.RenderStepped:Connect(function()
    if Humanoid and Humanoid.Health > 0 then
        Humanoid.WalkSpeed = VelAtual
    end
end)

-- Mensagem final
StarterGui:SetCore("SendNotification", {
    Title = "✅ TTP HUB v3.3 CARREGADO",
    Text = "Aimbot corrigido e funcionando!",
    Duration = 3
})
